...
def train_test(run_time, args):
    ...
    if args.model == 'gcnii':
        ...
        train_loader = DataLoader(train_dataset, batch_size=1, shuffle=True)
        val_loader = DataLoader(val_dataset, batch_size=args.val_test_batch_size, shuffle=False)
        test_loader = DataLoader(test_dataset, batch_size=args.val_test_batch_size, shuffle=False)
        non_hop_2_adjacency_matrix_dict_path = os.path.join("./data/PPI",
                                                            "non_hop_2_adjacency_matrix_dict_val_batch_size_{}.pkl".format(
                                                                args.val_test_batch_size))
        train_node_id_dict_path = os.path.join("./data/PPI",
                                               "train_node_id_dict.pkl")
        common_neighbors_dict_path = os.path.join("./data/PPI",
                                                  "common_neighbors_ppi.pkl")
        degree_dict_path = os.path.join(
            "./data/PPI",
            "degree_count_ppi.pkl"
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            for param in nodevae_optimizer.param_groups[0]['params']:
                ...
                """
                torch.nn.utils.clip_grad_value_(param, 5)
