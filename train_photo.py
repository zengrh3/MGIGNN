...
def train_test(run_time, args):
    ...
    if 'gcnii' not in args.model:
        ...
        transform = RandomNodeSplit(split='train_rest',
                                    num_val=3060, num_test=3060)
        photo_data = transform(dataset[0])
        photo_data = photo_data.to(device, 'x', 'y')

        train_edge_index = subgraph(photo_data.train_mask,
                                    photo_data.edge_index,
                                    relabel_nodes=True)[0]
        train_x = photo_data.x[photo_data.train_mask]
        train_y = photo_data.y[photo_data.train_mask]
        ...
        subgraph_loader = NeighborLoader(copy.copy(photo_data),
                                         input_nodes=None,
                                         num_neighbors=[-1], shuffle=False,
                                         **kwargs)
        subgraph_loader.data.num_nodes = photo_data.num_nodes
        subgraph_loader.data.n_id = torch.arange(photo_data.num_nodes)
        ...
        row, col, edge_attr = photo_data.adj_t.t().coo()
    ...
    num_nodes = photo_data.x.shape[0]
    ...
    model = MGIGNN(
        ...
        nodes_numbers=photo_data.x.shape[0],
