...
from gnn_encoder_subgraph import GraphSAGE_Encoder_SubGraph, \
    GAT_Encoder_SubGraph, \
    GCN_Encoder_SubGraph, \
    SGC_Encoder_SubGraph, \
    GCNII_Encoder_SubGraph, \
    GATv2Conv_Encoder_SubGraph, \
    FILM_Encoder_SubGraph, \
    DNA_Encoder_SubGraph, \
    GIN0_Encoder_SubGraph, \
    SSGC_Encoder_SubGraph, \
    FAGCN_Encoder_SubGraph
...
class NodeReprVAE(nn.Module):
    ...
    def encode(self, gnn_embedding, sim_info):
        ...
        #     gnn_embedding + self.delta * self.linear_layer(sim_info['most_similar_embedding']))
        batch_size = gnn_embedding.shape[0]

        self.mu_q_z = 0.01 * self.gnn_mean_q_z(
            gnn_embedding + self.delta * self.linear_layer(sim_info['most_similar_embedding']))
        # self.logstd_q_z = self.gnn_logstd_q_z(
        #     gnn_embedding + self.delta * self.linear_layer(sim_info['most_similar_embedding']))
        self.logstd_q_z = 0.01 * torch.ones(batch_size, 64).to(self.device)

        # Equation 8
        self.mu_p_z = 0.01 * self.gnn_mean_p_z(gnn_embedding)
        # self.logstd_p_z = self.gnn_logstd_p_z(gnn_embedding)
        self.logstd_p_z = 0.01 * torch.ones(batch_size, 64).to(self.device)
...
class MGIGNN_With_SubGraph(nn.Module):
    ...
    def __init__(self,
                 ...
                 device,
                 alpha=0.1):
        super(MGIGNN_With_SubGraph, self).__init__()
    ...
    def reconstruct_z_q(self, train_info, sim_info):
        ...
        # print("|> final_label_distribution is ", final_label_distribution)
